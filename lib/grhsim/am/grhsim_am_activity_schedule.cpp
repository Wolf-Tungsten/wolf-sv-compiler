...
namespace wolvrix::lib::grhsim::am
{
    ...
    scheduleGrhSimAmActivityBlocks(const GrhSimAmActivityScheduleInput &input, std::string &error)
    {
        ...
        for (std::size_t iteration = 0; iteration < kMaxCoarsenIterations && idlePassMask != 0x7;
             ++iteration) {
            rebuildClusterGraph();
            if (iteration == 0) {
        ...
        // greedy merging inside one commit-events bucket under the block limits.
                ...
                if (haveCurrent &&
                    (input.commitEventRank[atom] != input.commitEventRank[currentBucketAtom] ||
                     currentInstructions + input.atomInstructions[atom] >
                         input.maxCommitInstructionsPerBlock ||
                     currentStateWrites + input.atomStateWrites[atom] >
                         input.maxStateWritesPerBlock)) {
                    haveCurrent = false;
                }
                if (!haveCurrent) {
                    ++normalBlockCount;
