...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        std::cerr << "usage: grhsim-am-lower-json <design.json> [top] [--schedule] "
                     ...
                     "[--branchy-mux] [--resize-elision] [--inline-scalar-helpers] [--inline-scalar-divmod-helpers] [--inline-scalar-constants] [--branchless-activation] [--no-trace-comments] "
    ...
    for (int index = 2; index < argc; ++index)
    {
        ...
        else if (argument == "--inline-scalar-divmod-helpers")
        {
    ...
    try
    {
        ...
        if (runSchedule)
        {
            ...
            if (!emitDirectory.empty())
            {
                ...
                if (inlineScalarDivModHelpers)
                {
                    emitOptions.attributes.emplace("inlineScalarDivModHelpers", "true");
                }
                ...
                {
                    emitOptions.attributes.emplace("branchlessActivation", "true");
                }
